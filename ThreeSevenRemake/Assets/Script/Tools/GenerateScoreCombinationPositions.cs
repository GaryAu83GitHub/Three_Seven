...
public class GenerateScoreCombinationPositions
{
    ...
    public List<List<Vector2Int>> GetScorePositionListFrom(Vector2Int aStartPosition)
    {
        ...
        {
            if (GameSettings.Instance.EnableScoringMethods[(int)ScoreingLinks.LINK_2_DIGIT] && mScoreCombinationPositionAddons[index].Count == 2)
                positionList.Add(AddPositionToList(index, aStartPosition));
            if (GameSettings.Instance.EnableScoringMethods[(int)ScoreingLinks.LINK_3_DIGIT] && mScoreCombinationPositionAddons[index].Count == 3)
                positionList.Add(AddPositionToList(index, aStartPosition));
            if (GameSettings.Instance.EnableScoringMethods[(int)ScoreingLinks.LINK_4_DIGIT] && mScoreCombinationPositionAddons[index].Count == 4)
                positionList.Add(AddPositionToList(index, aStartPosition));
            if (GameSettings.Instance.EnableScoringMethods[(int)ScoreingLinks.LINK_5_DIGIT] && mScoreCombinationPositionAddons[index].Count == 5)
                positionList.Add(AddPositionToList(index, aStartPosition));
        }
